    ...
    <div class="row row-cards">
        ...
        <div class="col-12 col-xl-4"><div class="card h-100"><div class="card-header"><h2 class="card-title">Terima serial</h2></div><div class="card-body">
            <form method="POST" action="{{ route('serials.receive') }}" class="row g-3">@csrf
                ...
                <div class="col-12"><label class="form-label">Rack/bin (opsional)</label><select name="warehouse_location_id" class="form-select"><option value="">Tidak ditautkan</option>@foreach ($locations as $location)<option value="{{ $location->id }}">{{ $location->warehouse?->name }} · {{ $location->code }}</option>@endforeach</select><div class="form-hint">Harus berada di gudang yang dipilih.</div></div>
