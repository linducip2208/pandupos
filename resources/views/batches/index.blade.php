        ...
        <div class="col-12 col-xl-4">
            <div class="card h-100"><div class="card-header"><h2 class="card-title">Terima batch</h2></div><div class="card-body">
                <form method="POST" action="{{ route('batches.receive') }}" class="row g-3">@csrf
                    ...
                    <div class="col-12"><label class="form-label">Rack / bin (opsional)</label><select name="warehouse_location_id" class="form-select"><option value="">Tanpa lokasi spesifik</option>@foreach ($locations as $location)<option value="{{ $location->id }}">{{ $location->warehouse?->name }} · {{ $location->code }}</option>@endforeach</select></div>
