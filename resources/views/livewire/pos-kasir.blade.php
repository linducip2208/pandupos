            ...
            <div class="card-body p-0">
                <table class="table table-vcenter mb-0">
                    @foreach($cart as $i => $row)
                        <tr>
                            <td>{{ $row['name'] }}<div class="text-secondary">Rp {{ number_format($row['price'], 0, ',', '.') }} / {{ $row['unit_name'] }}</div><div class="small text-success">Sumber: {{ $row['price_source'] }}</div><select class="form-select form-select-sm mt-1" wire:change="changeUnit({{ $i }}, $event.target.value)" aria-label="Satuan {{ $row['name'] }}">@foreach($units as $unit)<option value="{{ $unit->id }}" @selected($unit->id===$row['unit_id'])>{{ $unit->short_name }}</option>@endforeach</select>@php($variantBatches = $batches->where('product_variant_id', $row['variant_id']))@if($variantBatches->isNotEmpty())<select class="form-select form-select-sm mt-1" wire:change="selectBatch({{ $i }}, $event.target.value || null)" aria-label="Batch {{ $row['name'] }}"><option value="">FEFO otomatis</option>@foreach($variantBatches as $batch)<option value="{{ $batch->id }}" @selected($batch->id === ($row['inventory_batch_id'] ?? null))>{{ $batch->batch_number }}{{ $batch->expires_at ? ' · '.$batch->expires_at->format('d/m/Y') : '' }}</option>@endforeach</select>@endif</td>
