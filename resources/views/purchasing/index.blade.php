...
<div class="row row-cards">
    <div class="col-12 col-xl-5"><div class="card h-100"><div class="card-header"><h2 class="card-title">Buat purchase order</h2><div class="card-actions"><a href="{{ route('purchasing.orders.create') }}" class="btn btn-sm btn-outline-primary">PO multi-baris</a></div></div><div class="card-body">
    ...
    <div class="col-12"><div class="card"><div class="card-header"><h2 class="card-title">Purchase return</h2><div class="card-actions">@if($purchases->whereIn('status',['partial','received'])->isNotEmpty())<a href="{{ route('purchasing.returns.create', $purchases->whereIn('status',['partial','received'])->first()) }}" class="btn btn-sm btn-outline-danger">Return multi-baris</a>@endif</div></div><div class="card-body"><p class="text-secondary">Jumlah return divalidasi terhadap barang yang telah diterima dikurangi seluruh return sebelumnya.</p><div class="row g-4">
        ...
        <div class="col-12 col-xl-7"><div class="table-responsive"><table class="table table-vcenter"><thead><tr><th>Return</th><th>PO</th><th>Nilai</th><th>Status</th><th>Alasan</th></tr></thead><tbody>@forelse($returns as $return)<tr><td><a href="{{ route('purchasing.returns.show', $return) }}">{{ $return->return_no }}</a></td><td>#{{ $return->purchase_id }} · {{ $return->purchase?->contact?->name }}</td><td>Rp {{ number_format((float)$return->total,0,',','.') }}</td><td><span class="badge">{{ $return->status }}</span><div class="small text-secondary">{{ str_replace('_',' ',$return->settlement_type) }}</div></td><td>{{ $return->reason }}</td></tr>@empty<tr><td colspan="5" class="text-center text-secondary py-4">Belum ada purchase return.</td></tr>@endforelse</tbody></table></div></div>
    </div></div></div></div>
</div>
