    ...
    public function dimensions(): array
    {
        return [
            ...
            'future_addon_parity' => $this->dimension([
                ['Accounting', 12, true, 'AccountingTest (10 tests/66 assertions): system chart bootstrap, balanced-journal enforcement, draft/post/void-with-reversal lifecycle, period-close guards, sales/purchase auto-posting inside source transactions (module-gated), trial balance/P&L/balance-sheet/cash-flow/tax/AR/AP reports, tenant isolation, RBAC, API lifecycle; docs/ACCOUNTING.md'],
                ...
                ['Asset Management', 6, true, 'AssetTest (8 tests/51 assertions): registration validation, straight-line + declining-balance schedules with salvage floor, transfer history, maintenance cycle, disposal gain/loss with disposed guards, module gating, tenant isolation, RBAC, API + workspace flows; docs/ASSET.md'],
                ['HRM', 6, true, 'HrmTest (8 tests/40 assertions): departments, unique employee codes, late-aware check-in/out with hour computation, leave balances/overlap/approval-stamped attendance, holidays, module gating, tenant isolation, RBAC, API + workspace flows; docs/HRM.md'],
                ['Payroll', 8, true, 'PayrollTest (9 tests/52 assertions): structure validation, snapshotted run math, guarded draft/approve/paid lifecycle, balanced payroll journal posting on approve when accounting is on (skipped when off), payslips, HRM dependency enforcement, tenant isolation, RBAC, API + workspace flows; docs/PAYROLL.md'],
                ['Ecommerce', 8, true, 'EcommerceTest (6 tests/44 assertions): published catalog visibility, email carts, checkout with stock validation, exact-total payment with single deduction and provenance, shipment lifecycle, slug storefront with suspension hiding, tenant isolation, RBAC, public + admin API; docs/ECOMMERCE.md'],
                ['WooCommerce connector', 6, true, 'WooCommerceTest (7 tests/38 assertions): encrypted credentials, product push/update via links, inventory push, idempotent order pull with unmapped-SKU failure isolation, customer upsert, signed webhook with replay safety, dependency enforcement, tenant isolation, RBAC, workspace flow; docs/WOOCOMMERCE.md'],
                ['HMS', 6, true, 'HmsTest (6 tests/44 assertions): patient/doctor registration, conflict-guarded scheduling, guarded visit lifecycle, single-record rule, pharmacy-validated invoices with once-at-paid dispense and provenance, module gating, tenant isolation, RBAC, API + workspace flows; docs/HMS.md'],
                ['Gym', 4, true, 'GymTest (7 tests/35 assertions): packages, single-active subscriptions, quota-capped check-ins with persistent lazy expiry, renewal as new history, payment balance rules, module gating, tenant isolation, RBAC, API + workspace flows; docs/GYM.md'],
                ['AI provider abstraction', 4, true, 'AiTest (7 tests/33 assertions): encrypted provider configs with validation, metered completions with pre-call budget enforcement, real-data anomaly detection, module gating, tenant isolation, RBAC, API + workspace flows; docs/AI.md'],
                ['Field Force', 4, true, 'FieldForceTest (7 tests/40 assertions): guarded task lifecycle, assignee-only check-in, coordinate validation, Haversine distance, validated private photo evidence, idempotency-first offline retries with race-safe unique keys, completion proof rules, module gating, tenant isolation, RBAC, API + workspace flows; docs/FIELDFORCE.md'],
                ['ZATCA / tax compliance', 4, true, 'ZatcaTest (7 tests/47 assertions): TLV encode/decode roundtrip, real QR SVG, UBL XML node assertions, hash, idempotent invoice generation, credit/debit notes, reporting ledger, tenant isolation, RBAC, API + workspace flows; docs/ZATCA.md (portal boundary explicit)'],
                ['Cheque', 4, true, 'ChequeTest (8 tests/44 assertions): receipt/issue validation with per-bank uniqueness, same-bank deposit batches, clearance with accounting posting, bounce with reason and redeposit, cancel guards, signed reconciliation, module gating, tenant isolation, RBAC, API + workspace flows; docs/CHEQUE.md'],
            ]),
            'tests' => $this->dimension([
                ['Regression suite', 60, true, 'Latest local full suite: 606 tests / 2516 assertions on MySQL canonical (1 mysql-only backup test skips on sqlite); MySQL CI matrix runs the same suite green'],
