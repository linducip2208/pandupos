...
class SerialWorkspaceController extends Controller
{
    public function index(Request $request): View
    {
        ...
        return view('serials.index', [
            'serials' => SerialNumber::query()->with(['variant.product', 'warehouse', 'inventoryBatch', 'purchase', 'salesInvoice', 'movements'])->when($status, fn ($query) => $query->where('status', $status))->latest()->limit(100)->get(),
    ...
    public function receive(Request $request, SerialNumberService $serials, AuditService $audit): RedirectResponse
    {
        ...
        $serial = $serials->receive($tenantId, (int) $data['warehouse_id'], (int) $data['product_variant_id'], $data['serial_number'], (float) $data['unit_cost'], $data['inventory_batch_id'] ?? null, $data['purchase_id'] ?? null);
