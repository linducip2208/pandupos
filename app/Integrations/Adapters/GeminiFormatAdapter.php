...
class GeminiFormatAdapter
{
    public function __construct(private readonly IntegrationProvider $provider) {}

    public function request(array $payload): array
    {
        throw_unless($this->provider->api_format === 'gemini_format' && $this->provider->is_active, InvalidArgumentException::class, 'Format provider tidak sesuai.');
