...
| Stock count/reconcile | Snapshot/review/approve/post count and read-only reconcile command | Count workspace | Full count API; CLI reconcile | `inventory.adjust` | Tenant warehouse/variant scope | Posting/idempotency/reconcile and requester-segregation proof | Present | PARTIAL |
