...
| Batch/expiry/FEFO | Batch workspace and goods receipt can create/select a batch; GRN line, stock ledger, supplier, PO and expiry provenance remain linked; controlled FEFO allocation | Tenant workspace and receipt-line lot inputs | Receive/expiry endpoints | Product policy only | Tenant references and batch/warehouse/variant ownership validated | FEFO/expiry plus GRN provenance, cross-tenant rejection and reversal regressions | Present | PARTIAL |
| Serial lifecycle | Audited receive/register UI with purchase/sale provenance and ledger history; sale checkout, return, void, and transfer receipt atomically keep serial status and exact ledger movement in sync | Tenant workspace and transfer-line serial selector | Receive/list plus serialized sale/transfer input | Product policy only | Tenant/invoice/serial/source-warehouse references validated | Duplicate sale plus checkout/return/void/transfer lifecycle proof | Present | PARTIAL |
...
| Transfer | Audited request/approve/ship/transit/partial-receive/receive/cancel state machine; requester separation, cost, and selected batch identity/provenance are preserved | Multi-line tenant request UI with optional source-batch selector, state actions and receipt inputs | Full workflow endpoints | `inventory.transfer` | Tenant warehouses, variants and source batches checked | Requester separation, partial receipt, no early destination stock, and batch preservation regressions | Present | PARTIAL |
| Adjustment/count/reconcile | Reasoned approve/post adjustment, snapshot/review/approve/post count, read-only reconcile command; requesters cannot self-approve adjustments or counts | Multi-line adjustment and count workspace | Full adjustment/count API; CLI reconcile | `inventory.adjust` | Tenant warehouse/variant scope | Posting/idempotency/reconcile and requester-segregation proof | Present | PARTIAL |
