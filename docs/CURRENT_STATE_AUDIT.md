...
| Serial lifecycle | Audited receive/register UI with purchase/sale provenance and ledger history; sale checkout, return and void atomically keep serial status and exact ledger movement in sync | Tenant workspace | Receive/list plus serialized sale input | Product policy only | Tenant/invoice/serial references validated | Duplicate sale plus checkout/return/void lifecycle proof | Present | PARTIAL |
