...
| Rack/bin | Optional zone/rack/shelf/bin master with audited lifecycle, reservation selector and location-linked movements | D | D | D | D | D | D | D | PARTIAL | Transfer, goods-receipt and POS location selectors |
...
| Transfer | Audited request/approve/ship/transit/partial-receive/receive/cancel state machine with preserved cost, selected-batch provenance, serials, and nonserial rack/bin source/destination trace | D | D | D | D | D | D | D | PARTIAL | Multi-line UI, requester/approver separation, batch, serial, and nonserial rack/bin regressions exist; serialized location trace and complete acceptance remain |
| Adjustment/count/reconcile | Audited approve/post adjustment, count snapshot-to-post, read-only reconcile CLI; requester cannot self-approve adjustment/count | D | D | D | D | D | D | D | PARTIAL | Location selectors and complete operational acceptance |
