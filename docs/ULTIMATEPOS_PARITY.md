...
| Serial tracking | Tenant receive/register UI, provenance/history, controlled lifecycle, ledger linkage and duplicate-sale prevention | D | D | P | P | D | D | D | PARTIAL | Document-driven return/transfer UI/integration, granular permission and end-to-end sales selector tests |
...
| Transfer | Audited request/approve/ship/transit/partial-receive/receive/cancel state machine with preserved cost and selected-batch provenance | D | D | D | D | D | D | D | PARTIAL | Multi-line request UI, requester/approver separation and batch preservation have regressions; location/serial selectors and complete acceptance remain |
