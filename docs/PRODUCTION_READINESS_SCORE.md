...
| Future addon parity | 52% (OPEN) | Accounting, CRM, Manufacturing, Repair, Project, Asset and HRM modules implemented with full evidence (`AccountingTest`, `CrmTest`, `MrpTest`, `RepairTest`, `ProjectTest`, `AssetTest`, `HrmTest` + docs); 9 addon gates (Payroll, Ecommerce, WooCommerce, HMS, Gym, AI, Field Force, ZATCA, Cheque) remain unverified and score only on real implementation |
