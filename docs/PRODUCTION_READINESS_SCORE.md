...
| Future addon parity | 60% (OPEN) | Accounting, CRM, Manufacturing, Repair, Project, Asset, HRM and Payroll modules implemented with full evidence (`AccountingTest`, `CrmTest`, `MrpTest`, `RepairTest`, `ProjectTest`, `AssetTest`, `HrmTest`, `PayrollTest` + docs); 8 addon gates (Ecommerce, WooCommerce, HMS, Gym, AI, Field Force, ZATCA, Cheque) remain unverified and score only on real implementation |
