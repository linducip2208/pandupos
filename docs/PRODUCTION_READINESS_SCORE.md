...
| Future addon parity | 40% (OPEN) | Accounting, CRM, Manufacturing, Repair and Project modules implemented with full evidence (`AccountingTest`, `CrmTest`, `MrpTest`, `RepairTest`, `ProjectTest` + docs); 11 addon gates (Asset, HRM, Payroll, Ecommerce, WooCommerce, HMS, Gym, AI, Field Force, ZATCA, Cheque) remain unverified and score only on real implementation |
