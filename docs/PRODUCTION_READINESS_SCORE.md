...
| Future addon parity | 34% (OPEN) | Accounting, CRM, Manufacturing and Repair modules implemented with full evidence (`AccountingTest`, `CrmTest`, `MrpTest`, `RepairTest` + docs); 12 addon gates (Project, Asset, HRM, Payroll, Ecommerce, WooCommerce, HMS, Gym, AI, Field Force, ZATCA, Cheque) remain unverified and score only on real implementation |
